...
static pa_card_profile *find_best_profile(pa_card *card) {
    ...
    PA_HASHMAP_FOREACH(profile, card->profiles, state) {
        ...
        if (!pa_card_profile_has_ports_with_availability(profile, PA_DIRECTION_OUTPUT, PA_AVAILABLE_YES))
            continue;
    ...
    if (pa_safe_streq(best_profile->name, "off")) {
        PA_HASHMAP_FOREACH(profile, card->profiles, state) {
            ...
            if (!pa_card_profile_has_ports_with_availability(profile, PA_DIRECTION_OUTPUT, PA_AVAILABLE_UNKNOWN))
                continue;
